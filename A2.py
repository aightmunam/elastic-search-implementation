...
class scoring:
	...
	def oktf(self, term, document):
		TF = self.tf_mem(term, document)
		return float(TF / (TF + 0.5 + 1.5 * (int(self.length(document)) / self.getAvgFieldLength())))
	...
	def tf_idf(self, term, document):
		return float(self.oktf(term, document) * math.log(TOTALDOCS / self.df(term)))
	...
	def K(self, document, k1, b):
		return float(k1 * ((1 - b) + (b * (self.length(document) / self.getAvgFieldLength()))))
	...
	def BM25(self, term, document, query):
		...
		k2 = 500
		b = 0.75
		tfd = self.tf_mem(term, document)
		tfq = self.tf_mem(term, query)
		return float(math.log((D + 0.5) / (self.df(term) + 0.5))) \
			   * (((1 + k1) * tfd) / (self.K(document, k1, b) + tfd)) \
			   * (((1 + k2) * tfq) / (k2 + tfq))
	...
	def Jelinek_Mercer_smoothing(self, term, document):
		lmbda = 0.6
		return lmbda * (self.tf_mem(term, document) / self.length(document)) + (
					(1 - lmbda) * (self.getCumulativeFrequency(term) / self.totalLength()))
